...
import 'package:phoenix_picker/time_picker/date_picker_constants.dart';
...
class SelectionEntity {
  ...
  bool isUnLimit() {
    return filterType == SelectionFilterType.unLimit;
  }

  /// 递归清空选中状态
  void clearSelectedEntity() {
    List<SelectionEntity> tmp = [];
    SelectionEntity node = this;
    tmp.add(node);
  ...
  List<SelectionEntity> currentTagListForEntity() {
    List<SelectionEntity> list = [];
    children.forEach((data) {
      if (data.filterType != SelectionFilterType.range &&
          data.filterType != SelectionFilterType.dateRange &&
          data.filterType != SelectionFilterType.dateRangeCalendar) {
        list.add(data);
      }
    });
    return list;
  ...
  /// [BrnSelectionFilterType.dateRange]
  /// [BrnSelectionFilterType.dateRangeCalendar]
  List<SelectionEntity> currentRangeListForEntity() {
    List<SelectionEntity> list = [];
    children.forEach((data) {
      if (data.filterType == SelectionFilterType.range ||
          data.filterType == SelectionFilterType.dateRange ||
