...
import 'selection_common_item_widget.dart';

/// 单列选择子组件
// ignore: must_be_immutable
class SelectionSingleListWidget extends StatefulWidget {
  late List<SelectionEntity> _selectedItems;
  ...
  @override
  _SelectionSingleListWidgetState createState() =>
      _SelectionSingleListWidgetState();
}

class _SelectionSingleListWidgetState extends State<SelectionSingleListWidget> {
  @override
      ...
      child: Container(
        ...
        child: ListView.separated(
          ...
          itemBuilder: (BuildContext context, int index) {
            SelectionEntity item = widget.items[index];

            /// 点击筛选，展开弹窗时，默认展示上次选中的筛选项。
            bool isCurrentFocused = isItemFocused(index, item);

            return SelectionCommonItemWidget(
              item: item,
              ...
              itemSelectFunction: (SelectionEntity entity) {
                if ((entity.filterType == SelectionFilterType.checkbox &&
                        !entity.isSelected) ||
                    entity.filterType != SelectionFilterType.checkbox) {
                  if (entity.hasCheckBoxBrother()) {
                    if (entity.isUnLimit() &&
                      ...
                      /// 同级别中，存在不限类型已经选中情况，选择非不限类型 item，不检查数量限制
                        ...
                        !SelectionUtil.checkMaxSelectionCount(entity)) {
                      PhoenixToast.show(
                          BrnIntl.of(context)
                              .localizedResource
                    ...
                    if (!SelectionUtil.checkMaxSelectionCount(entity)) {
                      PhoenixToast.show(
                          BrnIntl.of(context)
                              .localizedResource
                              .filterConditionCountLimited,
  ...
  bool isItemFocused(int itemIndex, SelectionEntity item) {
  ...
  /// Item 点击之后的数据处理
  void _processFilterData(SelectionEntity selectedEntity) {
    int totalLevel = SelectionUtil.getTotalLevel(selectedEntity);
    ...
    for (SelectionEntity item in widget.items) {
      if (item.isSelected) {
  ...
  void configOneLevelList(SelectionEntity selectedEntity) {
    if (SelectionFilterType.radio == selectedEntity.filterType) {
      /// 单选，清除同一级别选中的状态，则其他的设置为未选中。
      selectedEntity.parent?.clearChildSelection();
      selectedEntity.isSelected = true;
    } else if (SelectionFilterType.checkbox == selectedEntity.filterType) {
      /// 选中【不限】清除同一级别其他的状态
      if (selectedEntity.isUnLimit()) {
        selectedEntity.parent?.clearChildSelection();
        selectedEntity.isSelected = true;
  ...
  void configMultiLevelList(
      SelectionEntity selectedEntity, int currentListIndex) {
    /// 选中【不限】清除同一级别其他的状态
    if (selectedEntity.isUnLimit()) {
      ...
      selectedEntity.isSelected = true;
    } else if (SelectionFilterType.radio == selectedEntity.filterType) {
      /// 单选，清除同一级别选中的状态，则其他的设置为未选中。
      selectedEntity.parent?.children
          .where((f) => f != selectedEntity)
          .forEach((f) {
