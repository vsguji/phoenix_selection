...
import 'selection_menu_item_widget.dart';
import 'selection_range_widget.dart';
import 'selection_animate_widget.dart';
...
class BrnSelectionMenuWidget extends StatefulWidget {
  /// 筛选菜单数据
  final List<SelectionEntity> data;
...
class _BrnSelectionMenuWidgetState extends State<BrnSelectionMenuWidget> {
  ...
  List<SelectionEntity> result = [];
  List<String> titles = [];
  List<bool> menuItemActiveState = [];
  List<bool> menuItemHighlightState = [];
  SelectionListViewController listViewController =
  ...
  void initState() {
    ...
    for (SelectionEntity parentEntity in widget.data) {
  ...
  OverlayEntry _createEntry(SelectionEntity entity) {
      ...
      itemViewList.add(BrnSelectionMenuItemWidget(
        ...
        itemClickFunction: () {
          ...
          if (listViewController.isShow) {
            ...
            if (widget.data[index].filterType != SelectionFilterType.more &&
  ...
  bool _isRange(SelectionEntity entity) {
    if (SelectionUtil.hasRangeItem(entity.children) ||
        entity.filterShowType == SelectionWindowType.range) {
      return true;
    }
    var totalLevel = SelectionUtil.getTotalLevel(entity);
    if (totalLevel == 1 && entity.filterType == SelectionFilterType.checkbox) {
      return true;
    }
    return false;
  }

  Widget _createRangeView(SelectionEntity entity) {
    int? rowCount;
    if (widget.configRowCount != null) {
      rowCount = widget.configRowCount!(widget.data.indexOf(entity), entity) ??
          rowCount;
    }
    return RangeSelectionGroupWidget(
      entity: entity,
      ...
      onSelectionConfirm: (SelectionEntity result, int firstIndex,
  ...
  Widget _createSelectionListView(SelectionEntity entity) {
    /// 顶层筛选 Tab
  ...
  void _onConfirmSelect(SelectionEntity entity, SelectionEntity result,
  ...
  /// 筛选 Title 展示规则
  String? _getSelectedResultTitle(SelectionEntity entity) {
    /// 更多筛选不改变 title.故返回 null
    if (entity.filterType == SelectionFilterType.more) {
      return null;
    }
    if (PhoenixTools.isEmpty(entity.customTitle)) {
  ...
  String? _getTitle(SelectionEntity entity) {
    String? title;
    List<SelectionEntity> firstColumn =
        SelectionUtil.currentSelectListForEntity(entity);
    ...
    if (firstColumn.isNotEmpty) {
      for (SelectionEntity firstEntity in firstColumn) {
        secondColumn
            .addAll(SelectionUtil.currentSelectListForEntity(firstEntity));
        if (secondColumn.isNotEmpty) {
          for (SelectionEntity secondEntity in secondColumn) {
            thirdColumn
                .addAll(SelectionUtil.currentSelectListForEntity(secondEntity));
          }
        }
      }
    }
    ...
    if (firstColumn.isEmpty || firstColumn.length > 1) {
      ...
      if (firstColumn[0].isUnLimit()) {
        title = entity.title;
      } else if (firstColumn[0].filterType == SelectionFilterType.range ||
          firstColumn[0].filterType == SelectionFilterType.date ||
          firstColumn[0].filterType == SelectionFilterType.dateRange ||
          firstColumn[0].filterType == SelectionFilterType.dateRangeCalendar) {
        ...
        if (secondColumn.isEmpty || secondColumn.length > 1) {
          ...
          if (secondColumn[0].isUnLimit()) {
            title = firstColumn[0].title;
          } else if (secondColumn[0].filterType == SelectionFilterType.range ||
              secondColumn[0].filterType == SelectionFilterType.date ||
              secondColumn[0].filterType == SelectionFilterType.dateRange ||
              secondColumn[0].filterType ==
                  SelectionFilterType.dateRangeCalendar) {
            ...
            if (thirdColumn.isEmpty || thirdColumn.length > 1) {
              ...
              if (thirdColumn[0].isUnLimit()) {
                title = secondColumn[0].title;
                      ...
                      SelectionFilterType.range ||
                  thirdColumn[0].filterType == SelectionFilterType.date ||
                  thirdColumn[0].filterType == SelectionFilterType.dateRange ||
    ...
    title = PhoenixTools.isEmpty(joinTitle) ? title : joinTitle;
    return title;
  }

  String? _getDateAndRangeTitle(
      List<SelectionEntity> list, SelectionEntity entity) {
    String? title = '';
    if (!PhoenixTools.isEmpty(list[0].customMap)) {
      if (list[0].filterType == SelectionFilterType.range) {
        title =
            '${list[0].customMap!['min']}-${list[0].customMap!['max']}(${list[0].extMap['unit']?.toString()})';
      } else if (list[0].filterType == SelectionFilterType.dateRange ||
          list[0].filterType == SelectionFilterType.dateRangeCalendar) {
        title = _getDateRangeTitle(list);
      }
    } else {
      if (list[0].filterType == SelectionFilterType.date) {
  ...
  String _getDateRangeTitle(List<SelectionEntity> list) {
    String minDateTime = '';
    String maxDateTime = '';
  ...
  String? _getDateTimeTitle(List<SelectionEntity> list) {
    String? title = "";
  ...
  void _refreshSelectionMenuTitle(int index, SelectionEntity entity) {
    if (entity.filterType == SelectionFilterType.more) {
    ...
    if (entity.selectedListWithoutUnlimit().isNotEmpty) {
      menuItemHighlightState[index] = true;
    } else if (!PhoenixTools.isEmpty(entity.customTitle)) {
