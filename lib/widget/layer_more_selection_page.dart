...
/// 左侧是：二级筛选项 右侧是三级筛选项
/// 默认将第一个元素选中
class LayerMoreSelectionPage extends StatefulWidget {
  final SelectionEntity entityData;
  final SelectionConfig themeData;

  const LayerMoreSelectionPage({
    Key? key,
  ...
  @override
  _LayerMoreSelectionPageState createState() => _LayerMoreSelectionPageState();
}

class _LayerMoreSelectionPageState extends State<LayerMoreSelectionPage>
    with SingleTickerProviderStateMixin {
  List<SelectionEntity> _firstList = [];

  late List<SelectionEntity> _originalSelectedItemsList;

  ///当前选中的 一级筛选条件
  SelectionEntity? _currentFirstEntity;
      ...
      itemBuilder: (context, index) {
        return GestureDetector(
          onTap: () {
            ...
            if (_firstList[index].filterType == SelectionFilterType.radio) {
              setState(() {
                _currentIndex = index;
              ...
              _firstList[index].parent?.children.where((data) {
                return data.filterType != SelectionFilterType.checkbox;
              }).forEach((data) {
              ...
              if (!_firstList[index].isSelected) {
                if (!SelectionUtil.checkMaxSelectionCount(_firstList[index])) {
                  PhoenixToast.show(
                      BrnIntl.of(context)
                          .localizedResource
  ...
  Widget _buildLeftItem(int index) {
    ...
    List<SelectionEntity> list = _firstList[index].selectedList();
    //如果选中了不限 则展示 房山全部
    //如果选中了某几个
    //        如果可以跨区域 则显示数量 否则只加粗
  ...
  Widget _buildRightItem(int index) {
    bool isSingle = (_currentFirstEntity?.children[index].filterType ==
            SelectionFilterType.radio) ||
        (_currentFirstEntity?.children[index].filterType ==
            SelectionFilterType.unLimit);
          ...
          if (_currentFirstEntity != null) {
            if (isSingle) {
              ...
              if (!_currentFirstEntity!.children[index].isSelected) {
                if (!SelectionUtil.checkMaxSelectionCount(
  ...
  Widget _buildRightMultiItem(SelectionEntity? entity) {
    if (entity == null) {
  ...
  Widget _buildRightSingleItem(SelectionEntity? entity) {
    if (entity == null) {
      return const SizedBox.shrink();
    } else {
  ...
  void setInitialSecondShowingItem(SelectionEntity currentFirstEntity) {
    //设置初始化的二级筛选条件 -1没有
    ...
    if (secondIndex == -1 && currentFirstEntity.children.isNotEmpty) {
      for (int i = 0, n = currentFirstEntity.children.length; i < n; i++) {
        if (currentFirstEntity.children[i].isUnLimit() &&
            currentFirstEntity.filterType == SelectionFilterType.checkbox) {
