...
import 'selection_datepicker_animate_widget.dart';

const String _defaultDateFormat = 'yyyy年MM月dd日';

/// 日期范围选择，筛选组件的子组件
// ignore: must_be_immutable
class SelectionDateRangeItemWidget extends StatefulWidget {
  /// 筛选数据
  ...
  _SelectionDateRangeItemWidgetState createState() =>
      _SelectionDateRangeItemWidgetState();
}

class _SelectionDateRangeItemWidgetState
    extends State<SelectionDateRangeItemWidget> {
  SelectionDatePickerController _datePickerController =
  ...
  void onTextTapped(bool isMax) {
    ...
    DateTime? minSelectedDateTime = PhoenixTools.isEmpty(widget.item.customMap)
        ? null
        : DateTimeFormatter.convertIntValueToDateTime(
            widget.item.customMap!['min']);
    DateTime? maxSelectedDateTime = PhoenixTools.isEmpty(widget.item.customMap)
        ? null
        : DateTimeFormatter.convertIntValueToDateTime(
            widget.item.customMap!['max']);
    ...
    Widget content = DateWidget(
      ...
      onConfirm: (DateTime selectedDate, List<int> selectedIndex) {
        ...
        if (!isMax &&
            PhoenixTools.isEmpty(widget.maxTextEditingController.text)) {
          onTextTapped(true);
        }
        setState(() {});
      },
    ...
    return OverlayEntry(builder: (context) {
      return GestureDetector(
        ...
        child: Container(
          ...
          child: SelectionDatePickerAnimationWidget(
