...
class BrnSelectionRangeTagWidget extends StatefulWidget {
  /// tag 显示的文本
  @required
  final List<SelectionEntity> tagFilterList;
...
class _BrnSelectionRangeTagWidgetState
    ...
    for (int nameIndex = 0;
      ...
      GestureDetector gdt = GestureDetector(
          ...
          onTap: () {
            ...
            if (SelectionFilterType.checkbox == selectedEntity.filterType &&
                !selectedEntity.isSelected) {
              if (!SelectionUtil.checkMaxSelectionCount(selectedEntity)) {
                PhoenixToast.show(
                    BrnIntl.of(context)
                        .localizedResource
            ...
            SelectionUtil.processBrotherItemSelectStatus(selectedEntity);
  ...
  Widget _tagWidgetAtIndex(int nameIndex) {
    ...
    if (widget.tagFilterList[nameIndex].filterType ==
            SelectionFilterType.date &&
        !PhoenixTools.isEmpty(widget.tagFilterList[nameIndex].value)) {
