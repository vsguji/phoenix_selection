...
class MoreSelectionWidget extends StatefulWidget {
  //entity 是商圈、钥匙等
  final SelectionEntity selectionEntity;
  ...
  @override
  _MoreSelectionWidgetState createState() => _MoreSelectionWidgetState();
}

class _MoreSelectionWidgetState extends State<MoreSelectionWidget> {
  @override
  Widget build(BuildContext context) {
    //弹出浮层
    if (widget.selectionEntity.filterType == SelectionFilterType.layer ||
        widget.selectionEntity.filterType == SelectionFilterType.customLayer) {
      return FilterLayerTypeByMoreItemWidget(
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
  ...
  ///用于 range和 tag 之间通信
  late StreamController<EventByMoreItem> streamController;
  ...
  void initState() {
    ...
    streamController.stream.listen((event) {
      if (event is InputEventByMoreItem) {
        setState(() {
          if (!event.filter) {
            //将所有tag设置为未选中
  ...
  Widget _buildSelectionTag() {
    return GridView.count(
      ...
      children: widget.selectionEntity
          .currentShowTagByExpanded(isExpanded)
          .map((SelectionEntity data) {
        return GestureDetector(
          onTap: () {
            setState(() {
              if (data.filterType == SelectionFilterType.radio) {
                data.parent?.clearSelectedEntity();
                data.isSelected = true;
                //用于发送 标签点击事件
                streamController.add(SelectEventByMoreItem());
              } else if (data.filterType == SelectionFilterType.checkbox) {
                if (!data.isSelected) {
                  if (!SelectionUtil.checkMaxSelectionCount(data)) {
                    PhoenixToast.show(
                        BrnIntl.of(context)
                            .localizedResource
                ...
                data.parent?.children
                    .where((_) => _.filterType == SelectionFilterType.radio)
                    .forEach((f) => f.isSelected = false);
                data.isSelected = !data.isSelected;
                //用于发送 标签点击事件
                streamController.add(SelectEventByMoreItem());
              } else if (data.filterType == SelectionFilterType.date) {
  ...
  Widget _buildSingleTag(SelectionEntity entity) {
    bool isDate = entity.filterType == SelectionFilterType.date;

    String? showName;

    if (isDate) {
      if (PhoenixTools.isEmpty(entity.value)) {
...
class _MoreRangeWidget extends StatefulWidget {
  ...
  ///用于自定义的筛选条件 最大值最小值
  final SelectionEntity rangeEntity;
...
class __MoreRangeWidgetState extends State<_MoreRangeWidget> {
  ...
  void initState() {
    ...
    minController.addListener(() {
      if (widget.rangeEntity.filterType != SelectionFilterType.range) {
        return;
      }
    ...
    minFocusNode.addListener(() {
      if (minFocusNode.hasFocus) {
        widget.streamController?.add(InputEventByMoreItem(
            filter: false, rangeEntity: widget.rangeEntity));
      }
    });
    ...
    widget.streamController?.stream.listen((event) {
      if (event is SelectEventByMoreItem) {
        maxController.clear();
        minController.clear();
  ...
  @override
  Widget build(BuildContext context) {
    if (widget.rangeEntity.filterType == SelectionFilterType.dateRange) {
      return SelectionDateRangeItemWidget(
...
class FilterLayerTypeByMoreItemWidget extends StatefulWidget {
  //entity是 商圈
  final SelectionEntity selectionEntity;
  final OnCustomFloatingLayerClick? onCustomFloatingLayerClick;
  final SelectionConfig themeData;
...
class _FilterLayerTypeWidgetState
    extends State<FilterLayerTypeByMoreItemWidget> {
  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
          ...
          child: GestureDetector(
            onTap: () {
              if (widget.selectionEntity.filterType ==
                  SelectionFilterType.layer) {
                Navigator.of(context)
                    .push(PageRouteBuilder<SelectionEntity>(
                        opaque: false,
                        pageBuilder: (context, animation, second) {
                          return LayerMoreSelectionPage(
                  ...
                  SelectionFilterType.customLayer) {
                if (widget.onCustomFloatingLayerClick != null) {
                  ...
                  widget.onCustomFloatingLayerClick!(
                      ...
                      (List<SelectionEntity> customFloatingLayerParams) {
        ...
        Padding(padding: EdgeInsets.only(top: 15), child: Line())
      ],
    );
  }
  ...
  String _getCondition() {
    String tmp = "";
    //返回所有选中的
    List<SelectionEntity> selectedList = widget.selectionEntity.selectedList();
    ...
    //  1：滤掉不限
    //  2：滤掉有选中的非不限的叶子节点
    List<SelectionEntity> result = selectedList.where((value) {
...
/// tag 和 range 之间的通信
abstract class EventByMoreItem {}

/// tag点击的事件
class SelectEventByMoreItem extends EventByMoreItem {}

/// 输入框的事件:携带 自定义的筛选条件 和 过滤标识位
/// 由于点击标签之后，会清空筛选条件，清空的时候，textField的监听也会执行一遍，因此需要过滤
class InputEventByMoreItem extends EventByMoreItem {
  final SelectionEntity rangeEntity;
  final bool filter;

  InputEventByMoreItem({required this.rangeEntity, this.filter = false});
}
