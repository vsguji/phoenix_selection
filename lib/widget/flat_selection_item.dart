...
///页面类型保罗了：标题、选择框
class FlatMoreSelection extends StatefulWidget {
  /// 数据源
  ...
  /// 当[SelectionEntity.filterType]为[BrnSelectionFilterType.layer] or[BrnSelectionFilterType.customLayer]时
  /// 跳转到二级页面的自定义操作
  ...
  @override
  _FlatMoreSelectionState createState() => _FlatMoreSelectionState();
}

class _FlatMoreSelectionState extends State<FlatMoreSelection> {
  @override
...
class __FilterCommonTypeWidgetState extends State<_FilterCommonTypeWidget> {
  ...
  Widget _buildSingleTag(SelectionEntity data) {
    bool isDate = data.filterType == SelectionFilterType.date;
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        ...
        const Padding(padding: EdgeInsets.only(top: 15), child: Line())
      ],
    );
  }
